...
class GaussianAdapter(nn.Module):
    ...
    def forward(
        ...
        # Compose world-space rotations: R_world = R_c2w @ R_local, then back to quat.
        rotations_b = rotations.broadcast_to((*scales.shape[:-1], 4))
        local_rotmat = quaternion_to_matrix(rotations_b)
        # c2w_rotations: (..., 3, 3); local_rotmat: (..., 3, 3). Broadcasting matmul.
        world_rotmat = c2w_rotations @ local_rotmat
        world_rotations = matrix_to_quaternion(world_rotmat)
