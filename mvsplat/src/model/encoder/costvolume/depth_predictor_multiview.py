...
class DepthPredictorMultiView(nn.Module):
    ...
    def forward(
        ...
        # IGF stage-2: shallow-add projected rate_feat into refine_out, then re-run
        # cloned `to_gaussians` AND cloned `to_disparity` (merge_disparity) on the
        # modified input — so the merge head predicts its own opacity (essential
        # for L_io BCE gradient signal).
        merged_raw_gaussians = None
        refine_out_m = refine_out  # used by both merge heads + disparity
        if merge_head is not None:
            if rate_feat_proj is not None:
                refine_out_m = refine_out_m + rate_feat_proj
            merged_in = torch.cat(
                [refine_out_m, extra_info["images"], proj_feat_in_fullres], dim=1
            )
            merged_raw = merge_head(merged_in)
            merged_raw_gaussians = rearrange(
                merged_raw, "(v b) c h w -> b v (h w) c", v=v, b=b
            )
        ...
        if self.wo_depth_refine:
            ...
            # delta fine depth and density (stage-1)
        ...
        # IGF merge_disparity: predict merged densities from the rate-injected
        # refine_out_m so the merge head has its own opacity prediction.
        merged_densities = None
        if merge_disparity is not None and not self.wo_depth_refine:
            merged_delta_disps_density = merge_disparity(refine_out_m)
            _merged_delta_disps, merged_raw_densities = merged_delta_disps_density.split(
                gaussians_per_pixel, dim=1
            )
            merged_densities = repeat(
                F.sigmoid(merged_raw_densities),
                "(v b) dpt h w -> b v (h w) srf dpt",
                b=b,
                v=v,
                srf=1,
        ...
        return depths, densities, raw_gaussians, merged_raw_gaussians, merged_densities
