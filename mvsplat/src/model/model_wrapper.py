...
class OptimizerCfg:
    ...
    warm_up_steps: int
    cosine_lr: bool
    igf_lr_multiplier: float = 10.0  # IGF merge/rate_embed params get lr * this; others = lr.
...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # IGF stage-2 needs rendered alpha for the L_io BCE term.
        ...
        # IGF stage-2 L_io loss (EcoSplat Eq. 9): renders stage-1 ori_gaussians once
        # for the L_acc term, then calls IGFModule.compute_loss on the side-channel.
        last_igf = getattr(self.encoder, "last_igf", None) if igf_active else None
        if last_igf is not None:
            ori_output = self.decoder.forward(
                last_igf["ori_gaussians"],
                batch["target"]["extrinsics"],
                batch["target"]["intrinsics"],
                batch["target"]["near"],
                batch["target"]["far"],
            ...
            igf_loss = self.encoder.igf.compute_loss(
                last_igf["distill_infos"], output, ori_output
            )
        ...
        if (
            ...
            protect_rate = last_igf["protect_rate"] if last_igf is not None else None
            num_gs = gaussians.means.shape[1]
            print(
                ...
                f"psnr = {psnr_probabilistic.mean().item():.4f}; "
                f"protect_rate = {protect_rate}; "
                f"#GS = {num_gs}"
            )
    ...
    def configure_optimizers(self):
        ...
        igf_lr = base_lr * self.optimizer_cfg.igf_lr_multiplier
        igf_params, base_params = [], []
        for name, p in self.named_parameters():
            if not p.requires_grad:
                continue
            if (".igf." in f".{name}.") or name.startswith("encoder.igf.") \
                    or "igf_rate_proj" in name:
                igf_params.append(p)
            else:
                base_params.append(p)
        if igf_params:
            print(f"[optim] {len(igf_params)} IGF params @ lr={igf_lr:.2e} | "
                  f"{len(base_params)} base params @ lr={base_lr:.2e}")
            optimizer = optim.Adam(
                [
                    {"params": base_params, "lr": base_lr},
                    {"params": igf_params, "lr": igf_lr},
                ]
            )
        else:
            optimizer = optim.Adam(self.parameters(), lr=base_lr)
        if self.optimizer_cfg.cosine_lr:
            # OneCycleLR accepts a list of max_lr (one per param group).
            max_lrs = [pg["lr"] for pg in optimizer.param_groups]
            warm_up = torch.optim.lr_scheduler.OneCycleLR(
                            optimizer, max_lrs if len(max_lrs) > 1 else max_lrs[0],
