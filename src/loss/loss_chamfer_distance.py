...
class LossChamferDistance(Loss[LossChamferDistanceCfg, LossChamferDistanceCfgWrapper]):
    ...
    def forward(
        self,
        distill_infos, output, ori_output,
    ) -> Float[Tensor, ""]:

        if "alpha" not in distill_infos:
            return torch.tensor(0.0, device=output.alpha.device)

        gt_alpha = distill_infos["alpha"].detach()
        pred_alpha = distill_infos["learn_alpha"]
        
        # gt_cov = distill_infos["gt_cov"].detach()
        # pred_cov = distill_infos["pred_cov"]
        
        rendered_alpha = output.alpha # acc alpha map should be close to 1
        
        # ori_depth = ori_output.depth.detach()
        # pred_depth = output.depth
        
        # gt_pixel = distill_infos["pixel_pts"].float().detach()
        # gt_pixel = torch.clamp(gt_pixel, -1, 1)
        # sample_pred_cov = torch.nn.functional.grid_sample(pred_cov.permute(0,3,1,2), gt_pixel[:,None], align_corners=True, mode='nearest').permute(0,2,3,1).squeeze(1)
        # sample_pred_cov = sample_pred_cov.view_as(gt_cov)
        # valid_mask = distill_infos["valid_mask"].float().detach()
        # triu_mask = torch.triu(torch.ones_like(gt_cov), diagonal=0)
        
        # cov_delta = (((sample_pred_cov - gt_cov)**2) * triu_mask).flatten(-2,-1).mean(dim=-1)
        # cov_loss = reduce(cov_delta * valid_mask, 'b n -> ', 'sum') / (valid_mask.sum() + 1e-6)
        loss = 0.1 * F.binary_cross_entropy(pred_alpha, gt_alpha) + F.binary_cross_entropy(rendered_alpha, torch.ones_like(rendered_alpha)) 
        # + 0.01 * F.l1_loss(pred_depth, ori_depth) 
        # + 1e3 *cov_loss

        return self.cfg.weight * torch.nan_to_num(loss, nan=0.0)

# def sqrtm_spd(A, eps=1e-6):
#     # A: (..., 3, 3) SPD
#     w, V = torch.linalg.eigh(A)                      # eigenvalues/eigenvectors
#     w_clamped = torch.clamp(w, min=eps)
#     return (V * w_clamped.sqrt().unsqueeze(-2)) @ V.transpose(-2, -1)

# def bures_distance2(S1, S2, eps=1e-6):
#     # S1,S2: (..., 3, 3) SPD
#     S2_sqrt = sqrtm_spd(S2, eps)
#     inner = S2_sqrt @ S1 @ S2_sqrt
#     inner_sqrt = sqrtm_spd(inner, eps)
#     tr = torch.einsum('...ii->...', S1 + S2 - 2*inner_sqrt)
#     return tr.clamp(min=0.0)  # numerical safety
...
#     return cham_dist, cham_normals, cham_x_idx
...
#     cham_x, cham_norm_x, cham_x_idx = _chamfer_distance_single_direction(
#         x,
#         y,
#         x_lengths,
#         y_lengths,
...
#     else:
#         cham_y, cham_norm_y, cham_y_idx = _chamfer_distance_single_direction(
#             y,
#             x,
#             y_lengths,
#             x_lengths,
...
#             else:
#                 loss_normals = None
#     return _apply_batch_reduction(loss, loss_normals, weights, batch_reduction), cham_x_idx, cham_y_idx