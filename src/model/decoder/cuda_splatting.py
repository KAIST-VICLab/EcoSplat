...
def render_cuda(
    ...
    test_intr_i = intrinsics.clone()
    test_intr_i[:, 0] = intrinsics[:, 0] * w
    test_intr_i[:, 1] = intrinsics[:, 1] * h
    ...
    for i in range(b):
        ...
        image, rendered_alpha, _ = rasterization(gaussian_means[i], gaussian_rotations[i], gaussian_scales[i], gaussian_opacities[i], shs[i],
                                        view_matrix[i][None], test_intr_i[i][None], w, h, sh_degree=degree,
                                        # near_plane=near[i].mean(), far_plane=far[i].mean(),
                                        render_mode="RGB+D", packed=False,
                                        near_plane=1e-10,
                                        backgrounds=background_color[i].unsqueeze(0).repeat(1, 1),
                                        radius_clip=0.1,
                                        rasterize_mode='classic') # (V, H, W, 3) 
        image, rendered_depth = torch.split(image.squeeze(0).permute(2,0,1), [3, 1], dim=0)
