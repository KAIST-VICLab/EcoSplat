...
class ModelWrapper(LightningModule):
    ...
    def __init__(
        ...
        self.chamfer_loss = self.losses[-1]
        ...
        if self.encoder.cfg.estimating_pose:
            ...
            for target_view in range(v_tgt):
                ...
                with self.benchmarker.time("encoder"):
                    encoder_output = self.encoder(batch["context"], self.global_step, self.encoder.cfg.primitive_ratio, visualization_dump=visualization_dump, target=target_data)
            ...
            # Render Gaussians.
            with self.benchmarker.time("encoder"):
                encoder_output = self.encoder(batch["context"], self.global_step, self.encoder.cfg.primitive_ratio, visualization_dump=visualization_dump)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        visualization_dump = {}
        encoder_output = self.encoder(batch["context"], self.global_step, self.encoder.cfg.primitive_ratio, visualization_dump=visualization_dump, target=batch["target"] if self.encoder.cfg.estimating_pose else None)
    ...
    def render_video_generic(
        ...
        visualization_dump = {}
        encoder_output = self.encoder(batch["context"], self.global_step, self.encoder.cfg.primitive_ratio, visualization_dump=visualization_dump, target=None)
        gaussians = encoder_output['gaussians']
