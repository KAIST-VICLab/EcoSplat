...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # IGF stage-2: pull distill_infos from encoder. ori_output is not needed —
        # LossImportanceOpacity reads only `output.alpha` and `distill_infos`.
        ...
        # IGF L_io loss (paper Eq. 9). Active when encoder.cfg.igf is set.
        if (
            distill_infos is not None
            and getattr(self.encoder, "igf", None) is not None
        ):
        ...
        if (
            ...
            protect_rate = encoder_output.get("protect_rate")
            print(
