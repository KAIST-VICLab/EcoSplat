...
class DPTOutputAdapter_fix(DPTOutputAdapter):
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], imgs, image_size=None, conf=None, rate_feat=None):
        ...
        if rate_feat is not None and rate_feat.shape[1]==768:
            layers = [l+rate_feat if idx > 0 else l for idx, l in enumerate(layers)]
...
class PixelwiseTaskWithDPT(nn.Module):
    ...
    def forward(self, x, imgs, img_info, conf=None, rate_feat=None):
        out = self.dpt(x, imgs, image_size=(img_info[0], img_info[1]), conf=conf, rate_feat=rate_feat)
        if self.postprocess:
